...
#include "Better_dynamic_topography.h"
...
template <int dim> pair<string,string> BetterDynamicTopography<dim>::execute ( TableHandler & )
{
  ...
  return pair<string,string>("Computing better dynamic topography", "");
}

template <int dim> const ::aspect::LinearAlgebra::BlockVector & BetterDynamicTopography<dim>::topography_vector() const
{
  return topo_vector;
}

template <int dim> const Vector<float> & BetterDynamicTopography<dim>::cellwise_topography() const
{
  return visualization_values;
}

template <int dim> list<string> BetterDynamicTopography<dim>::required_other_postprocessors() const
{
  return std::list<std::string> (1, "boundary pressures");
}

template <int dim> void BetterDynamicTopography<dim>::output_to_file ( const bool upper, vector<pair<Point<dim>, double>> & stored_values )
{
  ...
  std::string filename = this->get_output_directory() + "Topography/" +
...
template <int dim> void BetterDynamicTopography<dim>:: declare_parameters ( ParameterHandler & prm )
{
...
template <int dim> void BetterDynamicTopography<dim>::parse_parameters ( ParameterHandler & prm )
{
  if ( Utilities::MPI::this_mpi_process ( this->get_mpi_communicator() ) == 0 )
...
template <int dim> void BetterDynamicTopography<dim>::save ( map<string, string> & status_strings ) const
{
...
template <int dim> void BetterDynamicTopography<dim>::load ( const map<string, string> & status_strings )
{
